...
class Store(object):
    def __init__(
        self, output_dir: str = "path", logger: Logger = getLogger("__model__")
    ):
        """
        Description:
        ...
        self.logs = None
        self.lengths = {"train": None, "valid": None, "test": None}
    ...
    def set_train_sliding_window(
        ...
        self.training_sliding_window = {
            ...
            "lengths": {
                ...
                "quantitatives": len(quantitatives)
                if quantitatives is not None
                else None,
                ...
                "session_idxs": len(session_idxs) if session_idxs is not None else None,
            },
        }
    ...
    def set_valid_sliding_window(
        ...
        self.valid_sliding_window = {
            ...
            "lengths": {
                ...
                "labels": len(labels) if labels is not None else None,
                "idxs": len(idxs) if idxs is not None else None,
                "session_labels": len(session_labels)
                if session_labels is not None
                else None,
            },
        }

    def get_valid_sliding_window(
        self,
        length=False,
        session_labels=False,
        sequence_idxs=False,
        labels=False,
        semantics=False,
        quantitatives=False,
    ...
    def set_test_sliding_window(
        ...
        self.test_sliding_window = {
            ...
            "lengths": {
                ...
                "semantics": len(semantics) if semantics is not None else None,
                "labels": len(labels) if labels is not None else None,
                "sequence_idxs": len(sequence_idxs)
                if sequence_idxs is not None
                else None,
                "test_sessionIds": len(test_sessionIds)
                if test_sessionIds is not None
                else None,
    ...
    def get_train_data(self, n=None, m=None):
        ...
        if n < 0:
            n = len(self.train_data) + n  # Handle negative start index

        if m < 0:
            m = len(self.train_data) + m  # Handle negative end index

        if (
            n < 0
            or m < 0
            or n > len(self.train_data)
            or m > len(self.train_data)
            or n > m
        ):
    ...
    def get_test_data(self, n=None, m=None, blockId=None):
        ...
        if n < 0:
            n = len(self.test_data) + n  # Handle negative start index

        if m < 0:
            m = len(self.test_data) + m  # Handle negative end index

        if (
            n < 0
            or m < 0
            or n > len(self.test_data)
            or m > len(self.test_data)
            or n > m
        ):
            raise ValueError("Invalid range")
    ...
    def get_valid_data(self, n=None, m=None):
        ...
        if n < 0:
            n = len(self.valid_data) + n  # Handle negative start index

        if m < 0:
            m = len(self.valid_data) + m  # Handle negative end index

        if (
            n < 0
            or m < 0
            or n > len(self.valid_data)
            or m > len(self.valid_data)
            or n > m
        ):
            raise ValueError("Invalid range")
    ...
    def set_false_positives(self, logs):
        ...
        new_false_positives = [log for log in logs if log[0] not in existing_ids]
